...
```
Se observan que solo las variables chk_acctA13, chk_acctA14, duration, credit_hisA34, purposeA41,purposeA42, purposeA43, purposeA49, amount, saving_acctA64, saving_acctA65, installment_rate, sexA93, other_debtorA103, other_installA143 y foreignA202 son todas significativas con un alfa del 5%. 
...
```
Una vez obtenido los coeficientes del segundo modelo, compararé estos con los correspondientes del primer modelo, enfocándome solo en las variables que son significativas para uno y otro modelo. Para eso obtengo los coeficientes y p valores de cada uno.
...
```
Se observa cierta correlación positiva entre duration y amount, ya que es de 0.62, por lo qhe está más cercana a uno que cero. Quizás sea por eso que es la variable significativa cuyo coeficiente más cambia al eliminar amount. En el caso de installment rate tiene una correlación  negativa de -0.27, pero no es tan fuerte como en el caso de duration.
...
En lo que respecta a las variables que solicita el ejercicio, se observa que hay 3 segmentos en la variable amount, los cuales pueden ser vistos gráficamente con el comando *plotmo*.
...
```
En el gráfico se observa entonces que hay un primer tramo donde crece la probabilidad de que el crédito sea malo a medida que aumenta el importe del crédito, luego aparece un punto de corte y sucede lo contrario: disminuye la probabilidad de que el crédito sea malo a medida que crece el monto prestado. El último segmento, que comienza en el monto 2978 es creciente con el monto del préstamo.
...
En virtud de que el modelo hallado en el ejercicio 4 tenía un AIC mayor, y en que parece lógicas las no linealidades encontradas por el algoritmo MARS, me quedo con el mismo para contestar las preguntas 5 hasta la 7.

Procedo a calcular la probabilidad estimada media de que el crédito sea malo para los mayores de 50. Para ello hago la predicción de la probabilidad para todas las observaciones, y luego hallo la media de dicha predicción para las personas de más de 50 años.

```{r cálculo de probabilidad de crédito malo para mayores de 50}
...
#Calculo la media de la probabilidad de que el créditos sea malo para los mayores de 50
german_credit %>% 
      dplyr::filter(age>50) %>% 
      summarise(
...
Esto es lo que predice el modelo, se puede hacer el cálculo de los datos reales, de cuál es la proporción de mayores de 50 con créditos malos.
...
```
Ahora realizaré k-fold cross validation para el modelo Ridge, usando 100 folds, a efectos de elegir el lambda óptimo.
...
plot(Ridge2)
```
Se observa en el gráfico que el logaritmo del lambda mínimo es de aproximadamente `r round(log(Ridge2$lambda.min),1)`, lo que equivale a que el lambda sea de `r round(Ridge2$lambda.min,3)`. Este lambda mínimo y los coeficientes resultantes con dicho lambda se presentan a continuación.
...
Resta comparar el modelo Ridge con el modelo creado con el algoritmo MARS y ver cuál de los dos tiene una mejor predicción. Para eso uso el área bajo la curva ROC
...
#Con modelo earth
pred_earth <- predict(modelo_logit_3,german_credit,type="response")
#Con modelo Ridge
pred_ridge <- predict(
            Ridge,
            type="response",
...
###Calculo valor de área bajo la curva ROC###

#Modelo earth
suppressMessages(auc(german_credit$response, pred_earth))

#Modelo Ridge
suppressMessages(auc(german_credit$response, pred_ridge))
```
El modelo Ridge present una mayor área bajo la curva ROC, por lo que tiene una mejor capacidad de predicción.
